...
def RRT(initial_Stance, goal_region, map, time_max):
    ...
    for i in tqdm(range(time_max)):
        ...
        time_step.append(round(time.time() - start_v_near, 6))
        #v_near = rrt_root if i==0 or not v_candidate else v_candidate
        #print('BEST_DISTANCE:',distance)
        time_step.append(round(time.time() - strat_1, 6))
        ...
        if selected_new_pose == None: 
            time_step.append(0)
            time_step.append(round(time.time() - strat_2, 6))
            time_step.append(0)
            continue # TODO maybe is better to break
        ...
        time_step.append(round(time.time() - start_trajectory, 6))
        ...
        if goal_check(v_candidate, goal_region):
            ...
            time_step.append(round(time.time() - start_3, 6))
            all_time.append(time_step)
            break

        time_step.append(round(time.time() - start_3, 6))

        #tot += time_step[1] + time_step[3] + time_step[4]
    ...
    print('\n### End RRT search ###')
    tot_for = time.time() - start_rtt
    #print("time rtt: ", tot_for)
    #print("time consumed registered: ", tot)
