...
def main(world_json, resolution, time_max, print_steps, no_display_steps, no_out, out, out2):
    ...
    # Planning
    steps_to_goal, all_steps, all_time, tot_for = RRT(initial_stance, goal_region, map, time_max)
    ...
    if(not no_out):
        ...
        totale_for = "Total time for the whole RRT cycles:"+ '\t' + str(round(tot_for, 7)) + '\n'
        f.write("\n")
        f.write(time_1)
        f.write(time_2)
        f.write(time_3)
...
def parse_opt():
    ...
    parser.add_argument('--world-json', type=Path, default='data/tunnel3.json', help='json file containing the information about the boxes contained in the world')
    parser.add_argument('--resolution', type=float, default=MLSM_RESOLUTION, help='Set the map resolution')
    parser.add_argument('--time-max', type=int, default=50000, help='Set the map resolution')
